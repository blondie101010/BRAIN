...
//Common::$debug = Common::DEBUG_WARNING;
Common::$debug = Common::DEBUG_INFO;
...
$argv0 = $argv[0];																			// keep it for usage below

// Take command line arguments WITHOUT VALIDATION, please call responsibly!
if ($argc > 1) {
...
$called = "php $argv0 inputFile=$inputFile [brainName=$brainName] [site=$site] [brainPath=$brainPath] [batchSize=$batchSize] [skip=$skip] [sample=$sample]";

if (is_null($inputFile) || !$batchSize || in_array($argv[1], ['--help', '-h'])) {
	Common::trace("usage: $called\n\n\twhere `skip` is the number of lines to skip, and `sample` means to get sample batches at $skip intervals when non-zero.\n", Common::DEBUG_ERROR);
	exit(1);
}

Common::trace("called as: $called\n", Common::DEBUG_INFO);
...
$testMode = file_exists("$brainPath/Brain-$brainName.dat");
...
$todoLineCount ++;
	...
	while ($this->status == 'O' && !empty($contents)) {
		$contents = $this->processDataBlock($contents, $testMode, [], "A");
		...
		if (count($contents)) {
			...
			$retries = 0;
			while (count($allFailed) && $retries < self::MAX_ATTEMPTS) {
				shuffle($allFailed);														// prevent order from limiting our learning
				$lastFailed = array_unique($this->processDataBlock($allFailed, $testMode, $lastFailed, "aF"));
			...
			if ($attempts > 5 || (count($contents) <= 0.02 * count($firstContents) && count($lastFailed) < 0.002 * count($firstContents))) {
				break;
			}
		}
	}
